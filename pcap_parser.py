...
output_uniflows_separately = False
...
for key in flow_list_dict:
    ...
    if pkt[6] != time_list[0]:
        # Manual check after change in script to validate that change didn't affect the old script parameters
        print('ERROR TIME LIST != FIRST PACKET TIME!')
        exit()
...
csv_columns = ['ip_src', 'ip_dst', 'prt_src', 'prt_dst', 
                        'proto', 'fwd_num_pkts', 'bwd_num_pkts',
                        'fwd_mean_iat', 'bwd_mean_iat', 'fwd_std_iat', 
                        'bwd_std_iat', 'fwd_min_iat', 'bwd_min_iat',
                        'fwd_max_iat', 'bwd_max_iat','fwd_mean_offset', 'bwd_mean_offset','fwd_mean_pkt_len', 
...
df_biflow.to_csv('biflow_' + output_file, sep=',', columns=csv_columns) 
